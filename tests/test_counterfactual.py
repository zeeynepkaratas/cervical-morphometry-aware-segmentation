...
def test_area_preserving_perturbation_exactness():
    ...
    assert valid is True, f"Perturbation failed: {reason}"
...
def test_deterministic_within_cell_matching_and_no_reuse():
    from experiments.run_counterfactual_analysis import match_counterfactual_pairs
    ...
    pairs_df, unmatched = match_counterfactual_pairs(shape_df, counter_df)
    
    assert len(pairs_df) == 2
    assert pairs_df.iloc[0]["shape_severity"] == "r2"  # dilation, r2
    assert pairs_df.iloc[1]["shape_severity"] == "r1"  # erosion, r1
...
def test_report_json_decision_consistency():
    ...
    assert json_path.exists(), "JSON interpretation missing"
